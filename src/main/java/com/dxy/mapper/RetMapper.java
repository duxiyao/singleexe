...
public interface RetMapper {
    ...
    (
    SELECT 商品ID,是否有百亿促销,百亿基础价提报时间,百亿满减专区,百亿自主降价自主降价,百亿长期216折扣活动名字,百亿长期216折扣活动力度,_226品牌限时折扣名称,
    (CASE WHEN 百亿长期216折扣活动名字 is not NULL AND CAST(库存 AS NUMERIC)<=10  THEN '0.95'
          WHEN 百亿长期216折扣活动名字 is not NULL AND  CAST(库存 AS NUMERIC)>10 AND CAST(库存 AS NUMERIC)<30 AND 百亿长期216折扣活动名字 like '%8%' THEN '0.9'
          WHEN 百亿长期216折扣活动名字 is not NULL THEN 百亿长期216折扣活动力度
     ELSE NULL END) as _226品牌限时折扣力度,_226品牌限时折扣活动价,百亿秒杀,月卡专享价7月
    from(
    SELECT a.商品ID,a.库存,
     max(CASE WHEN (CAST(a._1件实收最低场景商家预估实收价格 AS NUMERIC) - CAST(a.百亿基础价 AS NUMERIC))   = 0 THEN '只有百亿基础价，没有叠加其他促销' ELSE NULL END) as 是否有百亿促销,
    ...
    * */
    ...
    /**
     ...
     (
     SELECT 商品ID,是否有百亿促销,百亿基础价提报时间,百亿满减专区,百亿自主降价自主降价,百亿长期216折扣活动名字,百亿长期216折扣活动力度,_226品牌限时折扣名称,
     (CASE WHEN  _226品牌限时折扣名称 like '%8.5%' THEN '0.85'
     WHEN _226品牌限时折扣名称 like '%8%' THEN '0.8'
     WHEN _226品牌限时折扣名称 like '%9.5%' THEN '0.95'
     WHEN _226品牌限时折扣名称 like '%9%' THEN '0.9'
     ELSE NULL END) as _226品牌限时折扣力度,_226品牌限时折扣活动价,百亿秒杀,月卡专享价7月
     from(
     SELECT a.商品ID,a.库存,
     max(CASE WHEN (CAST(a._1件实收最低场景商家预估实收价格 AS NUMERIC) - CAST(a.百亿基础价 AS NUMERIC))   = 0 THEN '只有百亿基础价，没有叠加其他促销' ELSE NULL END) as 是否有百亿促销,
